...
// Metodo per deserializzare un Utenti
func (u *Utenti) UnmarshalJSON(data []byte) error {
	var aux map[string]interface{}
	if err := json.Unmarshal(data, &aux); err != nil {
		return err
	}

	u.SetNome(aux["Nome"].(string))
	u.SetCognome(aux["Cognome"].(string))
	u.SetEmail(aux["Email"].(string))
	u.SetPassword(aux["Password"].(string))
